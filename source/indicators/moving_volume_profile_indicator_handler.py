# indicators/moving_volume_profile_indicator_handler.py
...
class MovingVolumeProfileIndicatorHandler(VolumeProfileIndicatorHandler):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, (index, row) in enumerate(data.iterrows()):
            volume_profiles_data = super().calculate(data[max(0, i - self.__window_size) : i + 1])
            ...
            moving_volume_price_df.loc[index, 'mvp'] = volume_profiles_data[volume_profiles_connected_to_lower_prices]['volume'].iloc[-1]
